...
object ScreenToolFactory {
    fun registerDefaults(context: Context, registry: ToolRegistry) {
        ...
        registry.register(ScreenTool(ToolDefinition(
            name = "ler_tela",
        ...
        registry.register(ScreenTool(ToolDefinition(
            name = "tocar_a_tela",
            description = "Toca em um elemento da tela pelo texto ou descrição visível. Prefira a árvore de acessibilidade em vez de coordenadas.",
        ...
        registry.register(ScreenTool(ToolDefinition(
            name = "digitar_a_tela",
            description = "Digita texto em um campo editável identificado pelo texto ou descrição.",
            parameters = mapOf("alvo" to "texto ou descrição do campo", "texto" to "texto a inserir"),
    ...
    private fun findNodeRecursive(
        ...
        val matches = normalizedTarget.equals(text.trim(), ignoreCase = true) ||
            normalizedTarget.equals(description.trim(), ignoreCase = true) ||
            text.contains(normalizedTarget, ignoreCase = true) ||
            description.contains(normalizedTarget, ignoreCase = true)

        if ((!editableOnly || node.isEditable) && matches && node.isEnabled) return node
